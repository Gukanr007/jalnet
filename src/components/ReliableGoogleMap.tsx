...
export const ReliableGoogleMap: React.FC<ReliableGoogleMapProps> = ({
  ...
  const getZoomScale = useCallback((zoomLevel: number, assetType: string) => {
    ...
    if (assetType === 'pipe' || assetType === 'tap') {
      ...
      return minScale + (maxScale - minScale) * ((normalizedZoom - minZoom) / (maxZoom - minZoom));
    }
  ...
  const getAssetIcon = useCallback((asset: Asset, zoomLevel: number) => {
    ...
    const fillColor = conditionColors[asset.condition as keyof typeof conditionColors] || baseColors[asset.type as keyof typeof baseColors];
    const strokeColor = baseColors[asset.type as keyof typeof baseColors] || '#000000';
    ...
    const getBaseScale = (type: string) => {
      switch (type) {
        case 'tank': return 1.0;   // Small but visible
        case 'pump': return 1.0;   // Small but visible
        case 'valve': return 1.0;  // Small but visible
        case 'meter': return 2.5;  // Keep same for meters
        case 'tap': return 4;      // Keep same for taps
        case 'pipe': return 5;     // Keep same for pipes
        default: return 1.0;
      }
    };
  ...
  useEffect(() => {
    ...
    console.log('Setting up map click listener, placement mode:', isPlacementMode);
    ...
    if (isPlacementMode) {
      const listener = map.addListener('click', (event: google.maps.MapMouseEvent) => {
        if (event.latLng) {
          ...
          console.log('Map click detected during placement mode:', { lat, lng });
          onMapClick(lat, lng);
        }
      });
  ...
  const createPipelines = useCallback(() => {
    ...
    networkConnections.forEach(connection => {
      ...
      if (fromAsset && toAsset) {
        ...
        const config = lineConfig[connection.type as keyof typeof lineConfig] || lineConfig.service;
            ...
            content: `
              <div class="p-3">
                ...
                <div class="space-y-1 text-xs">
                  <p><span class="font-semibold">Type:</span> ${connection.type.charAt(0).toUpperCase() + connection.type.slice(1)}</p>
                  <p><span class="font-semibold">From:</span> ${fromAsset.name}</p>
                  <p><span class="font-semibold">To:</span> ${toAsset.name}</p>
                  <p><span class="font-semibold">Status:</span> Active</p>
                  <p><span class="font-semibold">Flow Direction:</span> ${fromAsset.name} → ${toAsset.name}</p>
                </div>
              </div>
            `,
            maxWidth: 250
    ...
    console.log(`Created ${newPolylines.length} pipeline connections with zoom scale ${baseLineScale.toFixed(2)}`);
  ...
  const createMarkers = useCallback(() => {
    ...
    console.log(`Creating ${uniqueAssets.length} markers (${realTimeAssets.length} real-time + ${assets.length} simulated) at zoom level ${currentZoom}...`);
    ...
    const newMarkers = uniqueAssets.map(asset => {
      ...
      const getAssetSpecificInfo = (asset: Asset) => {
        ...
        return typeDescriptions[asset.type as keyof typeof typeDescriptions] || 'Water infrastructure asset';
      };
      ...
      const getInfoWindowContent = (asset: Asset) => {
        let content = `
          <div class="p-3 min-w-[200px]">
            ...
            <div class="space-y-1 text-sm">
              <p><span class="font-semibold">Type:</span> ${asset.type.charAt(0).toUpperCase() + asset.type.slice(1)}</p>
              <p><span class="font-semibold">Function:</span> ${getAssetSpecificInfo(asset)}</p>
              <p><span class="font-semibold">Condition:</span> 
                <span class="inline-block w-2 h-2 rounded-full ml-1 mr-1" style="background-color: ${
                  asset.condition === 'good' ? '#22c55e' : 
                  asset.condition === 'average' ? '#f59e0b' : 
                  asset.condition === 'poor' ? '#ef4444' : '#dc2626'
                }"></span>
        ...
        if (asset.type === 'tap' && asset.water_connection) {
          ...
          if (asset.current_bill) {
            content += `
              <p><span class="font-semibold">Current Bill:</span> ₹${asset.current_bill.total_amount} (${asset.current_bill.status})</p>
              <p><span class="font-semibold">Due Date:</span> ${new Date(asset.current_bill.due_date).toLocaleDateString()}</p>`;
          } else {
        ...
        content += `
              ${asset.specifications ? `<p><span class="font-semibold">Specs:</span> ${Object.values(asset.specifications).join(', ')}</p>` : ''}
              ${asset.last_maintenance_date ? `<p><span class="font-semibold">Last Maintenance:</span> ${new Date(asset.last_maintenance_date).toLocaleDateString()}</p>` : ''}
            </div>
            <button onclick="window.selectAsset('${asset.id}')" class="mt-2 px-3 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700">
              View Details
            </button>
          </div>
        `;
    ...
    const avgScale = uniqueAssets.length > 0 ? getZoomScale(currentZoom, 'tank') : 0;
    console.log(`Created ${newMarkers.length} markers with zoom scale ${avgScale.toFixed(3)}`);
  }, [map, assets, realTimeAssets, onAssetClick, currentZoom, getAssetIcon, getZoomScale]);
  ...
  useEffect(() => {
    if (map && (assets.length > 0 || realTimeAssets.length > 0)) {
      console.log(`Zoom changed to ${currentZoom}, updating markers and pipelines...`);
      createMarkers();
      createPipelines();
    }
  }, [currentZoom]);
    ...
    return (
      <div className={`${className} flex items-center justify-center bg-red-50 rounded-lg border border-red-200`}>
        <div className="text-center p-6">
          ...
          <p className="text-xs text-red-500 mb-4">Retry attempt: {retryCount}</p>
          <Button 
            onClick={handleRetry}
            size="sm"
  ...
  if (isLoading) {
    return (
      <div className={`${className} flex items-center justify-center bg-blue-50 rounded-lg border border-blue-200`}>
        <div className="text-center p-6">
          <Loader2 className="h-12 w-12 animate-spin mx-auto text-blue-600 mb-4" />
          <p className="text-sm text-blue-600 font-medium">Loading Network Map...</p>
          <p className="text-xs text-blue-500 mt-2">Please wait while we initialize the map</p>
          {isPlacementMode && (
            <p className="text-xs text-green-600 mt-2">Ready for asset placement</p>
          )}
        </div>
      </div>
    );

  ...
  const createNetworkLines = useCallback(() => {
    ...
    networkConnections.forEach(connection => {
      ...
      if (fromAsset && toAsset) {
        const lineColor = connection.type === 'main' ? '#0066CC' : 
                         connection.type === 'secondary' ? '#00AA44' : '#666666';
        const lineWeight = connection.type === 'main' ? 6 : 
                          connection.type === 'secondary' ? 4 : 2;
          ...
          <div class="space-y-1 text-xs">
            ...
            <p><strong>Condition:</strong> 
              <span class="inline-block w-2 h-2 rounded-full ml-1" 
                    style="background-color: ${getConditionColor(asset.condition, assetConfig[asset.type].color)}"></span>
              ${asset.condition.charAt(0).toUpperCase() + asset.condition.slice(1)}
            </p>
            <p><strong>Location:</strong> ${asset.area}, ${asset.city}</p>
            ${asset.last_maintenance_date ? `<p><strong>Last Maintenance:</strong> ${asset.last_maintenance_date}</p>` : ''}
          </div>
        </div>
      `;

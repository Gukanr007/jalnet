...
export const WorkerDashboard = () => {
  ...
  const completedToday = tasks.filter(
    (t) =>
      t.status === "completed" &&
      t.updated_at === new Date().toISOString().split("T")[0]
  ).length;
    ...
    <div className="p-2 sm:p-4 space-y-3 sm:space-y-4 max-w-7xl mx-auto">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs sm:text-base text-gray-600">
          Assigned Area: Muthialpet • Pondicherry
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {tasks.map((task) => (
              <div
                key={task.id}
                className="p-2 sm:p-3 bg-gray-50 rounded-lg space-y-2"
              >
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs">
                    Due: {task.scheduled_date || "Not scheduled"}
                  </span>
                </div>
            ...
            {restockTasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <Badge
                    variant={
                      task.priority === "critical" ? "destructive" : "default"
                    }
                    className="text-xs"
                  >
                    {task.priority}
                ...
                <div className="space-y-1">
                  <p className="text-xs text-gray-600">
                    Qty: {task.requestedQuantity} units
                  </p>
                </div>
                <p className="text-xs text-gray-500 line-clamp-2">
                  {task.description}
